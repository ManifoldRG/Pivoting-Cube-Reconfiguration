"""
Fixed Configuration Demo Training Script

Train MAPPO agents on a fixed initial (plus-sign) to final (C-shape) configuration.
Generates GIFs only on successful episodes, with parallel movement visualization.

Features:
- Fixed configurations from demo_configs.py
- Success-only GIF generation
...
Usage:
    python -m train.train_fixed_demo --log_dir runs/demo --max_steps 500
...
from train.demo_configs import get_configs, NUM_AGENTS
...
def setup_logging(log_dir):
    ...
    logging.info("Fixed Demo Training - Logging initialized")
...
def train(args):
    ...
    logging.info("Initial (Plus): %s", init_conf)
    logging.info("Final (C-shape): %s", final_conf)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Train MAPPO on fixed plus-to-C configuration"
    )
    
    # Core parameters
    parser.add_argument('--max_steps', type=int, default=500,
                        help='Maximum steps per episode')
    parser.add_argument('--log_dir', type=str, default='runs/demo',
